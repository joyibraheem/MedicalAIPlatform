...
/// <summary>
/// Starts the local CheXNet FastAPI server (uvicorn) automatically when the Blazor app starts.
...
public sealed class CheXNetApiHostedService : IHostedService
{
    ...
    private async Task<bool> TryStartApiAsync(string host, string port, string baseUrl, CancellationToken cancellationToken)
    {
        ...
        var blazorDir = FindBlazorProjectRoot(contentRoot) ?? Directory.GetCurrentDirectory();
        var repoRoot = Directory.GetParent(blazorDir)?.FullName ?? blazorDir;
        var apiDir = Path.Combine(repoRoot, "CheXNet-master");
    ...
    private static string? FindBlazorProjectRoot(string start)
    {
        var dir = new DirectoryInfo(start);
        for (var i = 0; i < 10 && dir is not null; i++)
        {
            if (File.Exists(Path.Combine(dir.FullName, "CheXNet.BlazorApp.csproj")))
            {
                return dir.FullName;
            }
            dir = dir.Parent;
